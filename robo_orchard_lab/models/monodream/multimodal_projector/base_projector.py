...
def flat_square_3x3(x):
    ...
    if w % 3 != 0:
        x = torch.concat(
            [
                x,
                torch.zeros((n, 3 - (w % 3), h, c), dtype=x.dtype).to(
                    x.device
                ),
            ],
            dim=1,
    ...
    if h % 3 != 0:
        x = torch.concat(
            [
                x,
                torch.zeros((n, w, 3 - (h % 3), c), dtype=x.dtype).to(
                    x.device
                ),
            ],
            dim=2,
    ...
    def __init__(
        ...
        if mm_projector_type == "mlp_downsample_3x3_fix":
            ...
            if mlp_gelu_match:
                ...
                raise ValueError(
                    f"Unknown projector type: {mm_projector_type}"
                )
