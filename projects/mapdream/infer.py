...
def main():
    ...
    parser.add_argument("--progress-model-path", type=str, required=True)
    parser.add_argument("--action-model-path", type=str, required=True)
...
class NavAgent(Agent):
    def __init__(self, args, require_map=True):
        ...
        self.progress_model_pipline = ProgressModel.load_model(
            args.progress_model_path
        )
        self.progress_model_processor = ProgressModelProcessor(cfg=None)
        self.progress_model_pipline.to(device)

        self.action_model_pipline = InferencePipelineMixin.load_pipeline(
            args.action_model_path
        )
        self.action_model_pipline.model.init_components(args.action_model_path)
        self.action_model_pipline.model.to(device)
    ...
    def extract_actions_3step(self, output, max_steps=3):
        actions = []
        ...
        for seg in segments:
            ...
            if "stop" in seg:
                ...
                m = re.search(r"\d+", seg)
                if m:
                    actions.append((1, float(m.group())))

            elif "left" in seg:
                m = re.search(r"\d+", seg)
                if m:
                    actions.append((2, float(m.group())))

            elif "right" in seg:
                m = re.search(r"\d+", seg)
                if m:
                    actions.append((3, float(m.group())))
    ...
    def predict_inference(self, prompt):
        ...
        progress_data = self.progress_model_processor.pre_process(
            data=progress_data
        )
        gen_image = self.progress_model_pipline(progress_data)
        current_image_list.append(gen_image)
        data = ActionModelInput(
            image_paths=current_image_list,
            instruction=prompt,
        )
        response = self.action_model_pipline(data).text
        print(response)
        return response
    ...
    def act(self, observations, info, episode_id, img_save_dir):
        ...
        actions = self.extract_actions_3step(navigation[:-1])
        for action_index, num in actions:
            if action_index == 0:
