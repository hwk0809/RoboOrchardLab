...
def main():
    ...
    dataset_split = dataset.get_splits(
        args.split_num, sort_by_episode_id=True
    )[args.split_id]
...
class NavAgent(Agent):
    def __init__(self, args, require_map=True):
        ...
        self.progress_model_pipline = InferencePipelineMixin.load_pipeline(
            args.progress_model_path
        )
        self.progress_model_pipline.model.init_components(
            args.progress_model_path
        )
        self.progress_model_pipline.model.to(device)
    ...
    def predict_inference(self, prompt):
        ...
        if len(current_image_list) > 1:
            ...
            partial_instruction = self.progress_model_pipline(
                progress_data
            ).text
